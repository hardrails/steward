...
var completionTree = map[string][]string{
	...
	"agent deployment":            {"apply", "status", "list", "remove"},
	...
	"executor-command delegation": {"issue", "verify"},
	"control":                     {"pki", "tenant", "operator", "enrollment", "node", "node-credential", "operations", "attention", "agent", "command", "credential", "evidence", "evidence-capture"},
	"control pki":                 {"create"},
...
var completionFlags = map[string][]string{
	...
	"agent deployment apply":            {"-bundle", "-capsule", "-delegation", "-tenant", "-tenant-id", "-generation", "-revision", "-control-url", "-token-file", "-ca-file", "-no-context"},
