...
func newUpgradeFixture(t *testing.T) upgradeFixture {
	...
	writeUpgradeManifest(t, fixture.manifest, map[string]releaseFormatRange{
		"admission_fence":       {ReadMin: 1, ReadMax: 4, Write: 4},
		...
		"uplink_delivery_state": {ReadMin: 2, ReadMax: 4, Write: 4},
...
func writeUpgradeManifest(t *testing.T, path string, formats map[string]releaseFormatRange) {
	...
	if uplinkDelivery == (releaseFormatRange{}) {
		uplinkDelivery = releaseFormatRange{ReadMin: 2, ReadMax: 4, Write: 4}
	}
...
func TestUpgradeCheckDrainedSnapshotIsReadOnly(t *testing.T) {
	...
	want := "{\"signed_admission\":\"configured\",\"active_fences\":0,\"pending_operations\":0,\"retained_gateway_grants\":0,\"formats\":{\"admission_fence\":4,\"connector_receipt_log\":1,\"evidence_log\":null,\"gateway_state\":null,\"operation_journal\":null,\"supervisor_state\":1,\"uplink_delivery_state\":4,\"uplink_state\":2},\"target_compatible\":true,\"drained\":true}\n"
	...
	want = "{\"signed_admission\":\"configured\",\"formats\":{\"admission_fence\":4,\"connector_receipt_log\":1,\"evidence_log\":null,\"gateway_state\":null,\"operation_journal\":null,\"supervisor_state\":1,\"uplink_delivery_state\":4,\"uplink_state\":2},\"target_compatible\":true}\n"
	if output.String() != want {
...
func TestUpgradeCheckDrainedReportsAllLegacyFormats(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		`"operation_journal":1`, `"supervisor_state":1`, `"uplink_delivery_state":2`, `"uplink_state":1`,
	} {
...
func TestUpgradeManifestCompatibilityBlocksUnreadableObservedVersion(t *testing.T) {
	...
	var output bytes.Buffer
	err := run(fixture.arguments("check-drained", "configured"), &output, &bytes.Buffer{})
	if err == nil || !strings.Contains(err.Error(), "admission_fence version 4") || !strings.Contains(err.Error(), "choose a compatible release") {
	...
	writeUpgradeManifest(t, fixture.manifest, map[string]releaseFormatRange{
		"admission_fence":       {ReadMin: 1, ReadMax: 4, Write: 1},
		"connector_receipt_log": {ReadMin: 1, ReadMax: 1, Write: 1},
		"evidence_log":          {ReadMin: 1, ReadMax: 1, Write: 1},
		"gateway_state":         {ReadMin: 1, ReadMax: 3, Write: 3},
		"operation_journal":     {ReadMin: 1, ReadMax: 1, Write: 1},
...
func TestUpgradeManifestRequiresAndChecksUplinkDeliveryStateFormat(t *testing.T) {
	...
	formats["uplink_delivery_state"] = map[string]any{"read_min": 1, "read_max": 1, "write": 1}
	raw, err = json.Marshal(document)
	if err != nil {
		t.Fatal(err)
	}
	...
	output.Reset()
	err = run(fixture.arguments("inspect-formats", "configured"), &output, &bytes.Buffer{})
	if err == nil || !strings.Contains(err.Error(), "uplink_delivery_state version 4 is outside reader range 1-1") ||
		!strings.Contains(output.String(), `"uplink_delivery_state":4`) ||
		!strings.Contains(output.String(), `"target_compatible":false`) {
