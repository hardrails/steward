...
var completionTree = map[string][]string{
	...
	"control node-pool":        {"list", "status", "apply", "delete"},
...
var completionFlags = map[string][]string{
	...
	"control node-pool apply":           {"-pool-id", "-tenant-ids", "-architecture", "-min-nodes", "-desired-nodes", "-max-nodes", "-revision", "-control-url", "-token-file", "-ca-file", "-no-context"},
