...
version-2 or version-3 exact-request permit before DNS.
...
multi-party control: separate tenant keys sign the same exact request artifact,
and Gateway cannot resolve DNS until the signed threshold is complete.
...
| --- | --- | --- | --- | --- |
| Steward | One named connector operation and exact request bytes | Yes; 1 through 8 tenant-scoped Ed25519 authorities, with omission preserving one approver | Tenant, node, instance generation, capsule, site policy, route policy, operation policy, task, bytes, expiry, threshold, and signer set | Yes; the final permit and format-6 authorization/terminal receipts bind the canonical signer set and threshold |
| [NVIDIA NemoClaw / OpenShell](https://docs.nvidia.com/nemoclaw/0.0.32/network-policy/approve-network-requests.html) | A blocked host and port, with method and path shown when available; approval updates the running policy for the session | Not documented in the reviewed approval flow | Network-policy approval is session-scoped; an equivalent multi-signature exact-request artifact was not found | OCSF logs are documented; an equivalent signed multi-approver permit-to-terminal proof was not found |
...
### Multi-party Authorized Effects increment
...
Authorized grants prohibit generic egress, accept a version-2 single approval or a
version-3 threshold artifact for one exact request, keep the credential outside the
workload, durably spend authorization before DNS, and emit format-5 or format-6
evidence. “Exact” describes request bytes and metadata, not exactly-once upstream
delivery. Existing connectors retain their broader grant-and-task behavior until
the operator opts into this policy.
...
addresses abuse of authorized privilege without collusion. NemoClaw's documented
[network approval](https://docs.nvidia.com/nemoclaw/0.0.32/network-policy/approve-network-requests.html)
shows that operator prompts are becoming a standard sandbox feature; its approval
unit is a session network policy update, not a threshold signature over one exact
request. These sources motivate Steward's narrower control and do not certify it.
...
- [Open Agent Passport](https://arxiv.org/abs/2603.20953) proposes deterministic
  ...
  moat is signed tenant-key continuity through exact version-2 or threshold
  version-3 request authority,
