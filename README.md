...
| Excess inference attempts | An optional per-grant allowance atomically stops new provider attempts before network dispatch, across concurrent tasks and restart. Unknown outcomes and provider rejections still consume allowance. This is not a dollar spending cap. |
| Unbounded inference requests | An optional closed text-chat profile pins the provider model, supplies a hard output cap, permits only one default-tier completion and strips client provider overrides. Applications still own pricing and customer monetary reservations. |
...
| Host applications need signatures without keys | An optional [private signing station](docs/guides/task-signing-station.md) reuses native issuance for one operator-pinned runtime and operation allowlist, retaining exact bundles across retries. It does not dispatch or sign connector effects. |
...
adapter runs behind Steward's fixed security boundary. The v2 adapter includes
a bounded signed-operation-compatible stop path, deadline-bounded container
shutdown, and restart-safe gateway identity handling. Retained Linux/amd64 gVisor
evidence covers active-tool interruption and the complete signed integration.
...
- optional managed integration workers keep broker and provider credentials out of
  ...
  and Microsoft OneDrive metadata listing and selected text reads, fixed Gmail and
  Outlook inbox windows, upcoming Google and Outlook calendar windows, and a
  read-only HubSpot deal
  projection;
