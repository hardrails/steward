...
Current manifests declare `connector_receipt_log` with `read_min: 1`, `read_max: 9`,
and `write: 9`. Ordinary connector records retain schema 1. Action-permit records use
...
outcomes. It does not assert token usage, billing or task completion. All nine
