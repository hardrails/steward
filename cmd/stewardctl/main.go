...
var commandHelp = map[string]string{
	...
	"control":          "Enroll nodes, declare provider-neutral node-pool capacity, manage scoped operators, freeze command delivery during incidents, quarantine suspect snapshots, place and drain agent workloads, inspect agent events and task projections, verify fleet evidence, and create metadata-only support bundles.\n\nUsage: stewardctl control pki|tenant|operator|enrollment|node|node-pool|snapshot|operations|quota|freeze|attention|agent|event|task|command|credential|evidence|evidence-capture|support-bundle ...\n",
