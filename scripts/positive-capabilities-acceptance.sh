#!/usr/bin/env bash
# Real Docker+gVisor proof for state, inference, service, CLI/MCP, and receipts.
set -euo pipefail

: "${CAPABILITY_IMAGE:?set CAPABILITY_IMAGE to a local repository@sha256 agent image with /state owned by 65532}"
[[ $CAPABILITY_IMAGE == *@sha256:* ]] || { echo "CAPABILITY_IMAGE must be digest pinned" >&2; exit 2; }
...
relay_tag="steward-capability-relay-acceptance:$run_id"
...
manifest_digest=${CAPABILITY_IMAGE##*@}
repository=${CAPABILITY_IMAGE%@*}
config_digest=$(docker image inspect --format '{{.Id}}' "$CAPABILITY_IMAGE")
architecture=$(docker image inspect --format '{{.Architecture}}' "$CAPABILITY_IMAGE")
variant=$(docker image inspect --format '{{.Variant}}' "$CAPABILITY_IMAGE")
[[ $variant != '<no value>' ]] || variant=
platform="\"os\":\"linux\",\"architecture\":\"$architecture\""
[[ -z $variant ]] || platform+=",\"variant\":\"$variant\""
printf '%s\n' "{
 \"schema_version\":\"steward.admission.v1\",\"policy_id\":\"capability-acceptance\",\"policy_epoch\":1,
 ...
 \"tenants\":[{\"tenant_id\":\"$tenant_id\",\"publisher_key_ids\":[\"publisher\"],\"resource_ceiling\":{\"memory_bytes\":67108864,\"cpu_millis\":100,\"pids\":32},\"inference_route_ids\":[\"local-openai\"],\"inference_model_aliases\":[\"approved-model\"],\"service_ids\":[\"api\"]}]
}" >"$work/policy.json"
"$ctl_bin" policy sign -in "$work/policy.json" -out "$work/policy.dsse.json" -key "$work/site.private" -key-id site-root >/dev/null
printf '%s\n' "{
 \"schema_version\":\"steward.admission.v1\",\"capsule_id\":\"capability-agent\",\"publisher_key_id\":\"publisher\",\"profile\":{\"id\":\"generic-v1\",\"version\":\"v1\"},
...
"$executor_bin" -token-file "$work/token" -admission-policy-file "$work/policy.dsse.json" \
	...
	-gateway-control-socket "$work/gateway/control.sock" \
