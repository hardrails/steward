...
func gatewayCommand(arguments []string, stdout io.Writer) error {
	if len(arguments) == 0 {
		return errors.New("gateway command requires validate or route")
	}
	switch arguments[0] {
	case "validate":
		...
		if _, _, _, _, err := gateway.LoadConfig(*path); err != nil {
			return err
		}
...
func gatewayRouteCommand(arguments []string, stdout io.Writer) error {
	...
	if action == "list" {
		if *id != "" || len(destinations) != 0 || len(cidrs) != 0 {
...
func writeGatewayConfig(path string, config gateway.Config) error {
	...
	if _, _, _, _, err := gateway.LoadConfig(name); err != nil {
