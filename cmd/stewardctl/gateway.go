...
func gatewayServiceCommand(arguments []string, stdout io.Writer) error {
	...
	flags.SetOutput(io.Discard)
	path := flags.String("config", "/etc/steward/gateway.json", "gateway configuration")
	agent := flags.String("agent", "", "built-in agent preset: hermes or agent-service")
	...
	if *agent != "" {
		...
		if !ok {
			return errors.New("-agent must be hermes or agent-service")
		}
		*serviceID = contract.ServiceID
		operations = repeatedFlag{contract.OperationID + "=POST:" + contract.InvocationPath}
		lifecycles = repeatedFlag{contract.OperationID + "=" + contract.StatusPathPrefix}
	}
...
func gatewayAgentServicePreset(value string) (agentServicePreset, bool) {
	switch value {
	case "hermes":
		return agentServicePreset{
			ServiceID: "hermes-api", OperationID: "hermes.run",
			InvocationPath: "/v1/runs", StatusPathPrefix: "/v1/runs/",
		}, true
	case agentservice.RuntimeEngine:
		return agentServicePreset{
			ServiceID: agentservice.ServiceID, OperationID: agentservice.OperationID,
			InvocationPath: agentservice.InvocationPath, StatusPathPrefix: agentservice.StatusPathPrefix,
		}, true
	default:
...
func gatewayConnectorCommand(arguments []string, stdout io.Writer) error {
	...
	flags.SetOutput(io.Discard)
	path := flags.String("config", "/etc/steward/gateway.json", "gateway configuration")
	preset := flags.String("preset", "", "vetted preset: github-issues, research-search, research-extract, research-extract-v2, browser-search, browser-read, codex-worker, or claude-code-worker")
...
func applyAgentServiceConnectorPreset(
	...
	presets := map[string]connectorPreset{
		...
		"research-extract-v2": {
			id: "steward-research-extract-v2", operation: "extract=POST:/v2/extract",
			concurrent: 2, request: 64 << 10, response: 1 << 20, seconds: 60, calls: 64,
		},
		"browser-search": {
