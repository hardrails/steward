...
var completionTree = map[string][]string{
	"":                            {"help", "status", "explain", "recover", "console", "cluster", "site", "agent", "workroom", "context", "completion", "support", "keygen", "key", "capsule", "policy", "permit", "task", "executor-command", "control", "evidence", "node", "gateway", "secret", "image", "upgrade", "version"},
	"help":                        {"status", "explain", "recover", "console", "cluster", "site", "agent", "workroom", "context", "completion", "support", "keygen", "key", "capsule", "policy", "permit", "task", "executor-command", "control", "evidence", "node", "gateway", "secret", "image", "upgrade"},
	"console":                     {},
	"cluster":                     {"plan", "artifact", "baseline"},
	"cluster plan":                {"init", "join-server", "join-worker"},
	"cluster artifact":            {"bundle", "images", "checksums"},
	"status":                      {},
	"explain":                     {},
	"recover":                     {},
	...
	"task":                        {"run", "enqueue", "schedule", "list", "get", "result", "cancel", "issue", "serve-issuer", "verify", "audit", "submit", "status", "observe", "wait"},
	"task schedule":               {"create", "list", "show", "cancel"},
	"executor-command":            {"delegation", "issue", "verify"},
	"executor-command delegation": {"issue", "verify"},
	"control":                     {"pki", "backup", "tenant", "operator", "enrollment", "node", "node-pool", "node-credential", "snapshot", "operations", "quota", "freeze", "attention", "incident", "agent", "event", "task", "interaction", "command", "credential", "evidence", "evidence-capture", "support-bundle"},
	"control backup":              {"create", "verify", "restore"},
	"control pki":                 {"create"},
	...
	"control node":             {"list", "status", "assurance", "cordon", "uncordon", "quarantine", "unquarantine", "drain", "cancel-drain", "revoke"},
	"control node-pool":        {"list", "status", "apply", "delete", "membership-issue", "membership-verify", "membership-bind"},
	...
	"control event":            {"list"},
	"control task":             {"list"},
	"control interaction":      {"list", "show", "respond", "submit-response", "verify-response"},
...
var completionFlags = map[string][]string{
	...
	"cluster plan":                        {"-cluster", "-node", "-server", "-token-file", "-arch", "-air-gap", "-no-start", "-output"},
	"cluster plan init":                   {"-cluster", "-node", "-arch", "-air-gap", "-no-start", "-output"},
	"cluster plan join-server":            {"-cluster", "-node", "-server", "-token-file", "-arch", "-air-gap", "-no-start", "-output"},
	"cluster plan join-worker":            {"-cluster", "-node", "-server", "-token-file", "-arch", "-air-gap", "-no-start", "-output"},
	"cluster artifact":                    {"-arch", "-output"},
	"cluster artifact bundle":             {"-arch", "-output"},
	"cluster artifact images":             {"-arch", "-output"},
	"cluster artifact checksums":          {"-arch", "-output"},
	...
	"agent init":                          {"-runtime", "-template", "-name", "-force"},
	"agent create":                        {"-runtime", "-template", "-force"},
	...
	"agent deployment pause":              {"-tenant", "-tenant-id", "-revision", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"agent deployment resume":             {"-tenant", "-tenant-id", "-revision", "-control-url", "-token-file", "-ca-file", "-no-context"},
	...
	"task serve-issuer":                   {"-admission", "-intent", "-trust", "-key", "-key-id", "-store", "-operations", "-valid-for", "-capacity", "-socket", "-client-gid", "-allow-responses"},
	...
	"task get":                            {"-tenant-id", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"task result":                         {"-tenant-id", "-out", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"task cancel":                         {"-tenant-id", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"task schedule":                       {"-id", "-start-in", "-every", "-runs", "-window", "-max-concurrency", "-overlap", "-missed", "-project", "-session", "-instance-id", "-deployment-timeout", "-tenant-id", "-trust", "-key", "-key-id", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"task schedule create":                {"-id", "-start-in", "-every", "-runs", "-window", "-max-concurrency", "-overlap", "-missed", "-project", "-session", "-instance-id", "-deployment-timeout", "-tenant-id", "-trust", "-key", "-key-id", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"task schedule list":                  {"-tenant-id", "-after", "-limit", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"task schedule show":                  {"-tenant-id", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"task schedule cancel":                {"-tenant-id", "-control-url", "-token-file", "-ca-file", "-no-context"},
	...
	"workroom":                            {"-tenant-id", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"workroom create":                     {"-name", "-description", "-agent"},
	"workroom list":                       {"-after", "-limit"},
	"workroom delete":                     {"-revision"},
	"workroom session create":             {"-id", "-title"},
	"workroom artifact add":               {"-session", "-task", "-id", "-name", "-media-type", "-bytes", "-sha256", "-ref"},
	"workroom memory add":                 {"-id", "-title", "-artifact"},
	...
	"control node assurance":              {"-tenant-id", "-node-id", "-required-profile", "-max-age", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"control backup create":               {"-state-dir", "-out", "-no-context"},
	"control backup verify":               {"-in", "-no-context"},
	"control backup restore":              {"-in", "-state-dir", "-apply", "-no-context"},
	"control node-pool list":              {"-after", "-limit", "-control-url", "-token-file", "-ca-file", "-no-context"},
	...
	"control node-pool membership-issue":  {"-private-key", "-key-id", "-controller-id", "-pool-id", "-pool-membership-generation", "-pool-created-at", "-node-assurance", "-node-id", "-tenant-ids", "-architecture", "-boot-identity-sha256", "-scheduling-policy-sha256", "-runtime-assurance-sha256", "-valid-for", "-out", "-no-context"},
	...
	"control task list":                   {"-tenant-id", "-after", "-limit", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"control interaction list":            {"-tenant-id", "-after", "-limit", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"control interaction show":            {"-tenant-id", "-interaction-id", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"control interaction submit-response": {"-tenant-id", "-interaction-id", "-permit-file", "-response-file", "-control-url", "-token-file", "-ca-file", "-no-context"},
	"control interaction verify-response": {"-permit-file", "-response-file", "-public-key", "-key-id", "-max-validity", "-at", "-no-context"},
	"control interaction respond":         {"-tenant-id", "-interaction-id", "-choice", "-text", "-key", "-key-id", "-valid-for", "-clock-skew", "-control-url", "-token-file", "-ca-file", "-no-context"},
	...
	"gateway inference set":               {"-config", "-provider", "-id", "-base-url", "-protocol", "-upstream-model", "-max-tokens-cap", "-credential-file", "-credential-mode", "-anthropic-version", "-max-concurrent", "-require-attempt-receipts", "-disallow-attempt-receipts", "-require-task-scope", "-disallow-task-scope", "-max-calls-per-grant", "-disallow-call-limit", "-request-profile", "-disallow-request-profile"},
