...
var completionFlags = map[string][]string{
	...
	"gateway inference set":               {"-config", "-provider", "-id", "-base-url", "-protocol", "-upstream-model", "-max-tokens-cap", "-credential-file", "-credential-mode", "-anthropic-version", "-max-concurrent", "-require-attempt-receipts", "-disallow-attempt-receipts", "-require-task-scope", "-disallow-task-scope"},
