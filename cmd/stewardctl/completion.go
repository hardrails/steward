...
var completionFlags = map[string][]string{
	...
	"task enqueue":                        {"-bundle", "-instance-id", "-trust", "-task-id", "-valid-for", "-clock-skew", "-key", "-key-id", "-run-dir", "-deployment-timeout", "-tenant-id", "-control-url", "-token-file", "-ca-file", "-no-context"},
