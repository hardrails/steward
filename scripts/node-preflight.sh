...
image_pull_registry=${executor[EXECUTOR_IMAGE_PULL_REGISTRY]:-}
image_pull_auth=${executor[EXECUTOR_IMAGE_PULL_AUTH_FILE]:-}
if [[ -n $image_pull_auth && -z $image_pull_registry ]]; then
	echo "node-preflight: image pull authentication requires an approved registry" >&2
	exit 2
fi
if [[ -n $image_pull_registry ]]; then
	if (( admission_set != ${#admission_keys[@]} )); then
		echo "node-preflight: image pull requires complete signed admission" >&2
		exit 2
	fi
	image_pull_args=(
		-image-pull-registry "$image_pull_registry"
		-image-pull-auth-file "$image_pull_auth"
		-image-pull-timeout "${executor[EXECUTOR_IMAGE_PULL_TIMEOUT]:-5m}"
	)
fi
...
runuser -u steward-executor -- "$executor_bin" -check-config \
	...
	-operator-token-file "${executor[EXECUTOR_OPERATOR_TOKEN_FILE]:-}" \
	-observer-token-file "${executor[EXECUTOR_OBSERVER_TOKEN_FILE]:-}" \
