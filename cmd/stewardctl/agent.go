...
func agentCommand(arguments []string, stdout io.Writer) error {
	if len(arguments) == 0 {
		return errors.New("agent requires init, validate, build, plan, apply, fork, or doctor")
	}
	switch arguments[0] {
	case "init":
	...
	default:
		return fmt.Errorf("unknown agent command %q; expected init, validate, build, plan, apply, fork, or doctor", arguments[0])
	}
}
