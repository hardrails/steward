...
Current manifests declare `connector_receipt_log` with `read_min: 1`, `read_max: 10`,
and `write: 10`. Ordinary connector records retain schema 1. Action-permit records use
...
original task and target run without transferring run ownership. Schema 9 records
inference-attempt authorization and provider response or uncertain transport
outcomes. Schema 10 additionally binds an inference attempt to its admitted
service task, permit and request digests. Neither asserts token usage, billing,
semantic intent or task completion. All ten
schemas may appear in one signed chain. Configuring the closed stop operation
requires reader 8 prospectively. An inference route with
`require_attempt_receipts: true` requires reader 9 before its first receipt;
retained schema-9 records preserve that rollback boundary if accounting is later
disabled. A route with `require_task_scope: true` requires reader 10 before its
first receipt. Retained schema-10 records preserve that rollback boundary after
task scope is disabled. Format inspection
