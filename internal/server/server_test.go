...
func newTestHandler(maxInstances int) http.Handler {
	...
	return New(slog.New(slog.NewTextHandler(io.Discard, nil)), maxInstances, 0, false, nil).Handler()
}
...
func TestRecoverMiddlewareReturnsJSON500(t *testing.T) {
	s := New(slog.New(slog.NewTextHandler(io.Discard, nil)), 0, 0, false, nil)
...
func TestRequestLogCarriesRequestIDAndRemoteAddr(t *testing.T) {
	...
	h := New(slog.New(slog.NewJSONHandler(&buf, nil)), 0, 0, false, nil).Handler()
...
func TestListInstancesMatchesTrackerList(t *testing.T) {
	...
	h := NewWithTracker(slog.New(slog.NewTextHandler(io.Discard, nil)), tr, 0, false, nil).Handler()
