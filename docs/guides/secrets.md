...
Removal makes process restart fail closed: AppRole cannot authenticate again until
the operator issues and delivers a fresh one-use SecretID. The generated unit
limits failed restart attempts to three in five minutes instead of creating an
unbounded authentication loop. Before a planned restart or after a crash, install a
fresh SecretID, then run `systemctl reset-failed steward-openbao-agent` and restart
the service. Keeping a reusable SecretID on disk improves unattended availability
but weakens the hardened bootstrap boundary and is not generated by Steward.
...
3. Change the plan's `expected_version`, compile a new directory, and review the
   diff. For a value-only rotation, only `materialization.json` should change.
   Install it under a temporary name in the same directory and atomically rename it
   over the old manifest.
4. Let the running Agent retrieve the new static value on its configured interval,
   then wait until `materialization check` reports the expected and observed epochs
   equal. Do not restart merely to force a render: a restart needs a fresh one-use
   SecretID. An observed version ahead of the plan is a failed rollout, not
   implicit approval.
