...
> Source note: Reviewed 2026-07-19 using linked public primary sources. Product
...
transports tenant-signed commands and can reconcile exact desired instances by
signing only within a short-lived tenant delegation; it never holds a tenant
private key. It can also independently witness bounded, signed Executor evidence batches. A retained
checkpoint or sticky rollback or equivocation finding can be exported under a
separate witness key for offline verification. The online controller-signing,
evidence-witness, and TLS identities are purpose-separated. Evidence publication is
...
tenant intent, tenant signing keys kept outside Control, Executor-verifiable bounded
controller delegation, durable desired deployment generations, fenced exact-command delivery,
...
| Boundary | Read-only identity | Lifecycle authority | Admission authority | Tenant identity |
| --- | --- | --- | --- | --- |
...
| --- | --- | --- | --- | --- |
| Steward | One exact connector request, or an unordered set of up to eight exact requests; context locking is single-request only | Yes; 1 through 8 tenant-scoped Ed25519 authorities, with omission preserving one approver | Tenant, node, instance generation, capsule, site policy, route policy, every operation policy, task and request, expiry, threshold, signer set, and optionally the current signed connector-response history | Yes; the final artifact and format-5/6 authorization and terminal receipts bind each independently spent task to the same canonical authority; format 7 adds the history head and terminal response digest |
...
## Differentiation: approvals that expire when managed context changes

Exact request approval alone can become stale. An agent may receive another tool
response after approval but before dispatch. Steward's optional context-locked mode
binds the permit to the signed history of completed responses released through its
named connectors. Gateway serializes those calls and rejects the permit after the
history advances. The control is deterministic and outside the model, but covers
only Steward-mediated connector responses.

| Approach | Enforcement point | What makes an approval stale | Requires agent or planner changes | Offline reconstruction | Material limit |
| --- | --- | --- | --- | --- | --- |
| Steward context-locked effects | Gateway plus tenant-signed policy | Any later completed connector response for the same grant | No | Yes, from the signed connector ledger and an authenticated checkpoint | Excludes task input, inference, local memory, browser data, generic egress, and unmanaged channels; one in-flight connector call; no bundles |
| [PAuth](https://www.microsoft.com/en-us/research/publication/pauth-precise-task-scoped-authorization-for-agents/) | Agent authorization monitor | Task-scoped values or provenance do not satisfy the policy | Yes; integrates provenance-aware authorization | Not described as Steward-style portable signed receipts in the reviewed paper | Research prototype with a broader semantic provenance goal |
| [CaMeL](https://arxiv.org/abs/2503.18813) and [Fides](https://arxiv.org/abs/2505.23643) | Planner or agent data-flow enforcement | Information-flow or capability policy rejects the proposed tool call | Yes | Not described as Steward-style permit-to-terminal evidence | Stronger semantic flow tracking for participating agent stacks, not a transparent boundary for arbitrary containers |
| Browser origin isolation, such as [Google Agent Origin Sets](https://blog.google/security/architecting-security-for-agentic/) | Participating browser | Cross-origin read/write policy denies the browser action | Browser integration required | No equivalent signed effect-history artifact documented | Browser origins only; does not cover a general connector fleet |
| Network or application-protocol policy | Sandbox gateway | Destination, method, path, or protocol rule denies the call | Usually no | Product-specific logs | A permitted operation can still be motivated by newly hostile content |

The approaches are complementary. Planner information-flow controls can reason
about semantics that Steward deliberately does not interpret. Origin isolation can
reduce cross-site browser authority. Steward's narrower contribution is a portable,
framework-independent staleness rule for exact managed effects on disconnected
infrastructure.
...
| --- | --- | --- | --- | --- | --- | --- |
| Steward | Documented for a self-hosted controller and customer-owned Linux nodes, including air-gapped transfer | Bounded single-writer controller: scoped operators, one-time multi-tenant node enrollment with receipt-key proof, durable desired deployments, deterministic placement of exact delegated instances, and fenced delivery; tenant signing keys stay outside it and Executor independently verifies delegated controller commands | Documented for agent-service POSTs and connector methods/paths; Authorized Effects prohibits generic egress for the grant | Documented tenant keys scoped by signed policy to service IDs or connector IDs; policy can require distinct approvers over one exact request or a bounded exact set, or bind a single request to current signed connector-response history | Documented controller delivery fencing plus durable one-use connector spend before DNS and node-local task spend within one retained ledger epoch | Documented node-signed hash chain, format-5/6/7 authorized-effect evidence, independently retained controller checkpoint, sticky rollback/equivocation finding, controller-signed offline export, and offline task/permit correlation |
...
| [OpenClaw](https://github.com/openclaw/openclaw/security) | OpenClaw provides agents, tools, skills, memory, and optional Docker sandboxing. Its security documentation says one gateway is not an adversarial multi-tenant boundary and that session or memory scoping does not create per-user authorization. | Steward previously qualified a narrow OpenClaw surface, but active support is retired so the project can concentrate qualification and product depth on Hermes. OpenClaw remains a useful comparison, not a supported Steward runtime. |
...
Steward should reuse the understandable progression from useful task to
configuration, verification, and monitoring. It should not reuse hosted credential
custody, vendor-controlled evidence, or Internet-dependent operation.

The rows describe bounded Steward contracts. They do not imply a broad
...
| --- | --- | --- |
| Task-led onboarding | Start from a concrete Hermes task, show the exact adapter and capability boundary, and then introduce signed admission, task permits, and receipt verification only when the operator needs them. | Upstream agent behavior remains untrusted and independently versioned. Qualification evidence describes one pinned adapter and task; site policy and live admission still decide what may run. |
| Guided first action | Use one install/verify/admit/run/authorize/audit journey with saved CLI context, short lifecycle commands, a real custom skill, one exact task bundle, and one recoverable result path. | Keep task and action private keys off-node. Reuse the same task identity after an ambiguous submission rather than minting replacement authority. |
...
| Closed OpenClaw adapter | A second runtime consumes qualification, packaging, documentation, and security-review capacity while no active user depends on it. | Low without a concrete user requirement. | Low after retirement: Steward explicitly rejects the runtime and keeps only historical evidence. | High if retained: every upstream release and feature boundary must be requalified. | **Retired.** Concentrate the supported runtime surface on Hermes; reconsider only with a concrete operator need and a funded qualification contract. |
...
| Generic workflow engine | A manipulated plan gains another in-process execution path, or Steward duplicates agent behavior and expands its trusted core. | Medium: could simplify one product surface. | Low: Hermes and other agent frameworks already own planning, skills, and tool behavior. | Very high: broad semantics and integrations are difficult to bound or prove at the node boundary. | Reject from the Steward process. Keep qualifying external agents and enforce their authority outside them. |
...
| External secret storage with a provider-neutral Gateway handoff | A custom partial vault loses keys or recovery state; a broad provider token crosses tenants; permissive materialization creates group-readable or backup copies; a rotated value silently diverges from the expected epoch; or a secret reaches the agent or browser. | Steward prepares protected roots and validates a non-secret manifest, owner-only file identity, stable reads, and expected/observed rotation epochs while Gateway retains plaintext mediation outside workloads. The operator-selected secret manager owns provider authentication, encryption, ACLs, renewal, audit, and recovery. | Reuse an external open-source or existing operator service behind Steward's filesystem contract. Do not embed a provider client, generate one provider's lifecycle, or build a vault. |
...
| A real Hermes custom-skill effect | A health check or hard-coded fixture passes even though Hermes never discovers, loads, or follows the skill; a stale result is reused after restart. | Qualification requires Hermes's native system-prompt index, `skill_view` load of the exact signed `SKILL.md`, prescribed terminal call, one authenticated upstream effect, replay and forbidden-operation denial, changed persisted state after restart, secret-absence scans, and offline receipt verification. | Build and package the end-to-end qualification evidence. Treat retained evidence as test input, not a marketing assertion. |
| Retire the OpenClaw runtime | A no-user compatibility surface continues to consume release and security capacity. | The adapter, package payload, active docs, examples, CLI preset, and authoring enum are removed; older definitions fail closed. | Preserve historical evidence, publish a migration note, and require a fresh Hermes application rather than translating untrusted configuration. |
...
- The 2026 [MalSkillBench preprint](https://arxiv.org/abs/2606.07131) reports that
  ...
  code-and-instruction threat. Steward therefore pins exact adapter and skill bytes
  in qualification procedures while leaving semantic malware detection and
  behavioral safety claims outside its enforcement evidence. Exact-byte provenance
  reduces substitution risk; it does not prove that a skill is benign.
